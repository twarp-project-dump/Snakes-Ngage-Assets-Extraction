...
int read_sno_bin(const char *filename)
{
    ...
    // Read the last 16 bytes (checksum)
    unsigned char checksum[16];
    fseek(file, file_size - 0x10, SEEK_SET);
    fread(checksum, 1, 16, file);

    // Read the rest of the file
    size_t data_size = file_size - 0x10;
    ...
    for (int i = 0; i < data_size; i++)
    {
        ...
        file_data[i] ^= (state2 & 0xFF) + (state3 & 0xFF);
    }
    ...
    update_md5_w_qmemcpy(md5_state, (char *)file_data, data_size);

    unsigned char calculated_checksum[16];
    finish_md5_hash(calculated_checksum, md5_state);

    if (memcmp(checksum, calculated_checksum, 16) != 0)
    {
        printf("Note: checksum verification failed. Something probably went wrong, keep that in mind\n");
        free(file_data);
    } else{
...
int main(int argc, char *argv[])
{
    if (argc != 2)
    {
        printf("Usage: %s <sno.bin/sts.bin file>\n", argv[0]);
        return 1;
    }

    return read_sno_bin(argv[1]);
}